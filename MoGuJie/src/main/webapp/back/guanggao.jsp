...
<script>
	...
	$.post("advertise_getAllAdvertise",function(data){
		var obj=$("#weizhi");
		var obj2=$("#u_weizhi");
		var opt;
	...
	$.post("product_getAllProduct",function(data){
		var obj=$("#proname");
		var obj2=$("#up_proname");
		var opt;
		$.each(data.rows,function(index,item){
	...
	datagrid=$('#guanggao_info').datagrid({   
	    ...
	    columns:[[   
	        ...
	        {field:'gdates',title:'开始时间',width:100,align:'center'},
	        {field:'gdateo',title:'结束时间',width:100,align:'center'},
...
<script>
	...
	function showGuangGaoDetail(gid){
		...
		$.post("guanggao_findGuanggaoBygid",{gid:gid},function(data){
			$("#s_weizhi").val(data.weizhi);
			$("#s_gdates").val(data.gdates);
			$("#s_gdateo").val(data.gdateo);
			$("#s_gPhoto").val(data.gPhoto);
			$("#s_weight").val(data.weight);
			$("#s_click").val(data.click);
			$("#show_proname").val(data.proname);
			var str="";
			var pics=data.gPhoto.split(",");
			for(var i=0;i<pics.length;i++){
				str+="<img src='../"+pics[i]+"' width='100px' height='100px'>&nbsp;";
			}
